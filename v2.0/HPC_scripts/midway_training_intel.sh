...
#SBATCH -o midway_training_intel_%N.out
#SBATCH -e midway_training_intel_%N.err

# Training benchmark on Midway H200 (Intel Gold-6542Y, midway3-0602).
#
...
#   - 80 timed steps -> CSV at ${SLURM_SUBMIT_DIR}/midway_training_intel_bench.csv
#
...
echo "=== midway_training_intel: $(date -Iseconds) ==="
...
CSV_OUT="${SLURM_SUBMIT_DIR}/midway_training_intel_bench.csv"
...
torchrun --standalone --nproc_per_node=4 \
    "${SCRIPT}" \
    --yaml_config="${CONFIG}" \
    --run_num=train_h200_intel_$(date +%s)

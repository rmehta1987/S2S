...
### Real-Pangu training benchmark on Midway H200 (2026-05-22 update)

To close the inference-only loop, we then ran the **actual training benchmark** (not just inference) on both Midway H200 nodes using the same `train.py` and the same configuration that produced the H100 cluster baseline in this report (bfloat16, batch=2 per GPU, 20-step warmup + 80 measured steps, 4 GPUs). This run includes everything inference doesn't: the backward pass, gradient synchronisation across the four GPUs after every step, the real disk-backed data loader, and the optimiser update. If something about training specifically (the larger kernel mix, the cross-GPU communication, the dataloader cycling) caused the kind of pauses we see on DSI, it would show up here as long step-time outliers.

| Setup | Median step time | 90th-percentile step time | Step-to-step variation (std. dev.) | Throughput | Peak GPU memory |
|---|---:|---:|---:|---:|---:|
| H100 cluster (NVIDIA, "best confirmed" config from earlier in this report) | **1.146 s** | — | 0.005 s¹ | 6.98 samples/s | 69.0 GB |
| **Midway Intel H200** (`midway3-0602`) | **0.907 s** | 0.909 s | **0.0012 s** | **8.82 samples/s** | 69.1 GB |
| **Midway AMD H200** (`midway3-0601`) | **0.899 s** | 0.900 s | **0.0007 s** | **8.90 samples/s** | 69.1 GB |

¹ The H100 cluster row reports the standard deviation from the original `0.639 s` baseline (16-bit, batch=1, the cleanest H100 dataset we have); the "best confirmed" bfloat16/batch=2 row of the H100 summary table only recorded the median and throughput. So the Midway columns are the cleaner of the two for variation comparison.

**Two findings, both pointing the same way:**

1. **Midway H200 training is clean.** The 90th-percentile step time on both Midway nodes is within 2 milliseconds of the median (Intel: 0.909 − 0.907 = **0.0019 s** longest-of-ten-steps gap; AMD: **0.0010 s**). The step-to-step variation is **tighter than the H100 cluster's** — roughly 1 ms vs ~5 ms. If the DSI-style 10-50 ms pauses existed under training on Midway, they would appear as a long tail in this table; they don't. So the conclusion from the inference dispatch test extends end-to-end: Midway H200 stays clean under the full training workload, including the backward pass and the cross-GPU gradient sync.

2. **Midway H200 is about 21% faster than the H100 cluster at the same configuration.** 0.907 s vs 1.146 s per step, or 8.82 vs 6.98 samples per second. This is the magnitude you would expect from H200's higher memory bandwidth relative to H100 NVL when the workload is at all memory-bound. Both Midway nodes use the same configuration as the "best confirmed" H100 row (bfloat16 + the disabled unused-parameter search + batch=2 per GPU), so this is the most direct cross-cluster speed comparison in the report.

**What this says about DSI.** We have now ruled out, on Midway evidence alone: the H200 silicon itself; the 4-process distributed training setup; the absence of explicit memory-locality tuning; deep CPU sleep states (Midway only allows the shallow ones); the model's own kernel sequencing; the cross-GPU gradient sync; and the production data loader cycling. None of those reproduce the DSI behaviour on Midway. The remaining candidate causes are all DSI-host-specific: the GPU driver version, the GPU firmware, the kernel idle-state policy, the way interrupts are steered to CPU cores, or a co-tenant on the DSI node during the original profile capture. The diagnostic block in the "What is still unknown" section below is what would close this.

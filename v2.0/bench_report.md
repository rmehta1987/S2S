...
**What the Midway tests showed.** Midway has two types of H200 nodes — Intel CPU and AMD CPU. Both have direct high-speed connections between the four GPUs. The Intel node shows a ~3% slowdown when all four GPUs load data at the same time; the AMD node shows essentially no slowdown. A timing test on a single Midway H200 GPU showed the Python loop adds under a millisecond between forecast steps when running a synthetic model — but this is not directly comparable to DSI because the synthetic test has no data loading or file saving. We were also unable to record full hardware timelines on Midway H200 under a real four-GPU run due to no gpu-profiling available, so we cannot say whether DSI's pauses would appear there too. 
